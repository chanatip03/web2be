...
class DockerBuilder:
    ...
    def _render_static_frontend_dockerfile(self, rel_path: str) -> str:
        ...
        lines = [
            ...
            "  fi; \\",
            "fi",
            "# Support SPA client-side routing by falling back to index.html",
            "RUN printf 'server { listen 80; location / { root /usr/share/nginx/html; index index.html; try_files $uri $uri/ /index.html; } error_page 500 502 503 504 /50x.html; location = /50x.html { root /usr/share/nginx/html; } }' > /etc/nginx/conf.d/default.conf",
            "RUN apk add --no-cache curl",
            "EXPOSE 80",
