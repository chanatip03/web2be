...
class DockerBuilder:
    ...
    def _normalize_frontend_build_deps(self, dockerfile_content: str) -> str:
        """Ensure frontend build stages keep dev dependencies when needed.

        Many frontend projects (Vite, CRA, Next build) keep build tools in
        devDependencies. If Dockerfile uses production-only install, `npm run build`
        can fail with "vite: not found".
        """
        content = dockerfile_content or ""
        lower = content.lower()
        if "npm run build" not in lower:
            return dockerfile_content

        content = re.sub(r"(?m)^(\s*RUN\s+npm\s+ci)\s+--only=production\s*$", r"\1", content)
        content = re.sub(r"(?m)^(\s*RUN\s+npm\s+ci)\s+--omit=dev\s*$", r"\1", content)
        content = re.sub(r"(?m)^(\s*RUN\s+npm\s+ci)\s+--production\s*$", r"\1", content)
        content = re.sub(r"(?m)^(\s*RUN\s+npm\s+install)\s+--only=production\s*$", r"\1", content)
        content = re.sub(r"(?m)^(\s*RUN\s+npm\s+install)\s+--omit=dev\s*$", r"\1", content)
        content = re.sub(r"(?m)^(\s*RUN\s+npm\s+install)\s+--production\s*$", r"\1", content)
    ...
    def _harden_nginx_dockerfile(self, dockerfile_content: str) -> str:
        ...
        # Strip user-creation blocks that re-create nginx user.
        # This fails on nginx:alpine with "user 'nginx' in use".
        content = re.sub(
            r"(?ims)^\s*RUN\s+[^\n]*\badduser\b[^\n]*\bnginx\b[^\n]*(?:\\\s*\n\s*[^\n]*)*\n?",
            "",
            content,
        )
        # Remove possible orphan continuation lines left by malformed LLM output.
        content = re.sub(
            r"(?im)^\s*(chown|chmod)\b[^\n]*\\?\s*$",
            "",
            content,
        )
    ...
    async def build_and_run(
        ...
        for service_name, content in dockerfile_contents.items():
            ...
            if service_name == "frontend":
                ...
                content = self._normalize_frontend_build_deps(content)
    ...
    async def _build_with_repair(
        ...
        if service_name == "frontend":
            current_content = self._normalize_frontend_build_deps(current_content)
